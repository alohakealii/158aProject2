...
int lost[6] = {-1, -1, -1, -1, -1, -1};
unsigned long totalLatency[6];
...
int main(int argc, char *argv[])
{
    ...
    for (i = 0; i < rounds; i++) {
        printf("For %2dKB::  Packets lost:%2d   Total latency: %lu\nAverage latency: %lu\n", KB[i], lost[i], totalLatency[i], totalLatency[i]/100);
    }
    close(sockfd);
    return 0;
}
        ...
        if (length != lastLength) {
            ...
            for (i = 0; i < 6 && flag == 0; i++) {
                if (lost[i] == -1) {
                    ...
                    printf("For %2dKB:    Start time: %lu.%lu\nFinish time: %lu.%lu\n", KB[i], startTime.tv_sec, startTime.tv_usec, finishTime.tv_sec, finishTime.tv_usec);
